...
describe('RazorpayConfiguration', () => {
    ...
    describe('Rendering', () => {
        it('should render configuration form', async () => {
            ...
            await waitFor(() => {
                expect(screen.getAllByText(/Razorpay Configuration/i)[0]).toBeInTheDocument();
            });

            expect(screen.getByPlaceholderText(/rzp_test_.../i)).toHaveValue('rzp_test_abc123');
    ...
    describe('Form Interaction', () => {
        ...
        it('should toggle visibility of secrets', async () => {
            ...
            await waitFor(() => {
                expect(screen.getByPlaceholderText(/Enter your key secret/i)).toHaveAttribute('type', 'password');
            });

            const toggleBtns = screen.getAllByRole('button', { name: /👁️‍🗨️/i }); // Button text is actually emoji
            await user.click(toggleBtns[0]);

            expect(screen.getByPlaceholderText(/Enter your key secret/i)).toHaveAttribute('type', 'text');
        });
    });
    ...
    describe('Actions', () => {
        ...
        it('should handle test setup action', async () => {
            ...
            await waitFor(() => {
                expect(screen.getByRole('button', { name: /Test with Dummy Payment/i })).toBeInTheDocument();
            });

            const testBtn = screen.getByRole('button', { name: /Test with Dummy Payment/i });
