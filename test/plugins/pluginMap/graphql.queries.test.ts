...
describe('Plugin Map GraphQL Queries', () => {
    ...
    describe('getExtensionPointsOverviewResolver', () => {
        it('should return overview data', async () => {
            const result = await getExtensionPointsOverviewResolver({}, {}, mockCtx as any);
        ...
        it('should throw error when unauthenticated', async () => {
            ...
            await expect(getExtensionPointsOverviewResolver({}, {}, mockCtx as any))
    ...
    describe('getPluginMapRequestsResolver', () => {
        it('should fetch requests with pagination', async () => {
            mockCtx.drizzleClient.select.mockReturnValue({
            ...
            const result = await getPluginMapRequestsResolver(null, args, mockCtx as any);
            expect(result.requests.length).toBe(1);
            expect(result.totalCount).toBe(1);
        });

        it('should handle organizationId: null', async () => {
            const result = await getPluginMapRequestsResolver(null, { input: { organizationId: null } }, mockCtx as any);
            expect(result.requests).toBeDefined();
        });

        it('should throw error when unauthenticated', async () => {
            mockCtx.currentClient.isAuthenticated = false;
            await expect(getPluginMapRequestsResolver({}, {}, mockCtx as any))
                .rejects.toThrow(TalawaGraphQLError);
        });
        ...
        it('should handle invalid input', async () => {
            ...
            await expect(getPluginMapRequestsResolver(null, args as any, mockCtx as any))
                .rejects.toThrow(TalawaGraphQLError);
        });
        ...
        it('should handle database error', async () => {
            ...
            await expect(getPluginMapRequestsResolver(null, {}, mockCtx as any))
                .rejects.toThrow(TalawaGraphQLError);
        });
    });
    ...
    describe('getPluginMapPollsResolver', () => {
        it('should fetch polls with filters', async () => {
            ...
            const result = await getPluginMapPollsResolver(null, args, mockCtx as any);
            expect(result.polls).toBeDefined();
            expect(result.totalCount).toBeDefined();
        });

        it('should handle organizationId: null', async () => {
            const result = await getPluginMapPollsResolver(null, { input: { organizationId: null } }, mockCtx as any);
            expect(result.polls).toBeDefined();
        });

        it('should throw error when unauthenticated', async () => {
            mockCtx.currentClient.isAuthenticated = false;
            await expect(getPluginMapPollsResolver({}, {}, mockCtx as any))
                .rejects.toThrow(TalawaGraphQLError);
        });
        ...
        it('should handle invalid input', async () => {
            ...
            await expect(getPluginMapPollsResolver(null, args as any, mockCtx as any))
                .rejects.toThrow(TalawaGraphQLError);
        });
        ...
        it('should handle database error', async () => {
            ...
            await expect(getPluginMapPollsResolver(null, {}, mockCtx as any))
                .rejects.toThrow(TalawaGraphQLError);
        });
