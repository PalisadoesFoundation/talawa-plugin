...
const mockRequests = [
    createMockRequest({ id: 'req-1', pollNumber: 1, userRole: 'user', extensionPoint: 'RU1', organizationId: 'org-test', userId: 'test-user-id' }),
];
...
describe('ExtensionPointsUser', () => {
    ...
    it('should render the dashboard for a specific organization', async () => {
        ...
        expect(screen.getByTestId('pagination-total')).toHaveTextContent(/1-1 of 1 requests/i);
    });
    ...
    it('should redirect if orgId is missing', async () => {
        ...
        expect(screen.queryByText('RU1 - User Organization Extension Point')).not.toBeInTheDocument();
    });
    ...
    it('should use unknown-user if userId is missing in localStorage', async () => {
        ...
        await waitFor(() => {
            expect(message.success).toHaveBeenCalledWith(expect.stringContaining('77'));
        });
