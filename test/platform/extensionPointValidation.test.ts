...
describe('validateExtensionPoints', () => {
    ...
    it('should pass given valid file and export', async () => {
        ...
        vi.mocked(fs.existsSync).mockReturnValue(true);
        // Mock correct export
        vi.mocked(fs.readFileSync).mockReturnValue('export const myQuery = () => {};');
    ...
    it('should detect duplicate names', async () => {
        ...
        vi.mocked(fs.existsSync).mockReturnValue(true);
        vi.mocked(fs.readFileSync).mockReturnValue('export const query1 = {}; export const query2 = {};');

        const result = await validateExtensionPoints(manifest, mockPluginRoot);
        expect(result.valid).toBe(false);
        expect(result.errors).toContain('Duplicate extension name "duplicateName" found');
    });
});
