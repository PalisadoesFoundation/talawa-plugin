...
const UserTransactions: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (searchText) {
      filtered = filtered.filter(transaction =>
        ...
        transaction.organizationName.toLowerCase().includes(searchText.toLowerCase()) ||
    ...
    if (organizationFilter !== 'all') {
      filtered = filtered.filter(transaction => transaction.organizationId === organizationFilter);
    }
  ...
  const getUniqueOrganizations = () => {
    const orgs = transactions.map(t => ({ id: t.organizationId, name: t.organizationName }));
    return orgs.filter((org, index, self) => 
