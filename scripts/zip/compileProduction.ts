...
const ALLOWED_EXT_RE = /\.(?:js|cjs|mjs|json|node|wasm|css|scss|sass|map\.json|svg|png|jpg|jpeg|gif|webp|ico|ttf|otf|woff|woff2|eot|html|txt)$/i;
  ...
  while (stack.length) {
    ...
    for (const ent of entries) {
      ...
      if (ent.isDirectory()) {
        ...
        const zipPath = (prefix ? `${prefix}/` : '') + name.replace(new RegExp(sep.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), '/');
...
export async function compileForProduction(plugin: PluginInfo, skipTypeCheck: boolean = false): Promise<void> {
  const { execSync } = await import('node:child_process');
  const { existsSync, rmSync, mkdirSync, readdirSync, statSync } = await import('node:fs');
  ...
  try {
    ...
    if (plugin.hasAdmin && existsSync(join(plugin.path, 'admin'))) {
      ...
      writeFileSync(adminTsConfig, JSON.stringify(adminTsConfigContent, null, 2));
      ...
      try {
        ...
        try {
          ...
          if (skipTypeCheck) {
            console.warn('⚠️  TypeScript compilation failed, but continuing due to skip type check option');
            ...
            execSync(`find . -name "*.ts" -o -name "*.tsx" | xargs -I {} sh -c 'cp "{}" "dist/$(echo "{}" | sed "s/\\.ts$/.js/" | sed "s/\\.tsx$/.jsx/")"'`, { cwd: adminPath });
          } else {
            throw compileError;
          }
        }
        ...
        const distPath = join(adminPath, 'dist');
        if (existsSync(distPath)) {
          execSync(`find . -name "*.json" -o -name "*.css" -o -name "*.scss" -o -name "*.svg" -o -name "*.png" -o -name "*.jpg" -o -name "*.jpeg" -o -name "*.gif" -o -name "*.webp" | xargs -I {} cp --parents {} dist/`, { cwd: adminPath });
      ...
      try {
        ...
        try {
          ...
          if (skipTypeCheck) {
            ...
            // Copy TypeScript files as JavaScript (fallback)
            execSync(`find . -name "*.ts" | xargs -I {} sh -c 'cp "{}" "dist/$(echo "{}" | sed "s/\\.ts$/.js/")"'`, { cwd: apiPath });
          } else {
            throw compileError;
          }
        }
        ...
        const distPath = join(apiPath, 'dist');
        if (existsSync(distPath)) {
          execSync(`find . -name "*.json" -o -name "*.graphql" | xargs -I {} cp --parents {} dist/`, { cwd: apiPath });
    ...
    console.log(`✅ TypeScript compilation completed successfully${skipTypeCheck ? ' (type checking skipped)' : ''}`);
  } catch (error) {
...
export async function createProductionZip(plugin: PluginInfo, opts: ZipRoot = {}): Promise<string> {
  ...
  for (const base of ['manifest.json', 'package.json', 'README.md', 'LICENSE']) {
  ...
  console.log(`📊 Approx size: ${(statSync(zipPath).size / 1024 / 1024).toFixed(2)} MB`);
